...
**** Vue
***** Vue-Mode
Create a major mode that derives from ~web-mode~. This is mostly to uniquely
identify vue buffers for the lsp server.
...
***** LSP
Use ~volar~ / ~vue-language-server~ in eglot
...
****** Eglot
# Add vue-language-server to eglot-server-program
# #+begin_src elisp
...
# Disable auto import calculation
# #+begin_src elisp
...
# Add settings for ~vue-semantic-provider~ (Volar).
...
# # eglot has issues with lisp eval depth, set the depth higher as a workaround.
# #+begin_src elisp
...
**** Nil
Nil, an lsp server, is an alternative to rnix, which is automatically installed
in eglot and installable in lsp-mode. Lsp-mode should add nil support in version
8.0.1, but for now, it needs to be manually installed for eglot.
